    ...
    else
    {
        ...
        foreach($rank_list as $rank)
        {
            ...
            if($players_with_rank_number > 0)
            {
                ...
                {
$main_content .= '';
                    $main_content .= '<TR><TD><FORM ACTION="?subtopic=guilds&action=change_nick&name='.urlencode($player->getName()).'" METHOD=post><A HREF="?subtopic=characters&name='.urlencode($player->getName()).'">'.($player->isOnline() ? "<font color=\"\">".$player->getName()."</font>" : "<font color=\"\">".$player->getName()."</font>").'</A> <br><small>Level: '.$player->getLevel().' '.$vocation_name[$player->getWorld()][$player->getPromotion()][$player->getVocation()].'</small>';
