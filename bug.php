...
if($logged)
{
    ...
    if($group_id_of_acc_logged >= $config['site']['access_admin_panel'] and $_REQUEST['control'] == "true")
    {
        ...
        if(!empty($_REQUEST['id']) and is_numeric($_REQUEST['id']) and !empty($_REQUEST['acc']) and is_numeric($_REQUEST['acc']))
        {
            ...
            {
                if($bug[2]['status'] == 2)
                    $value = "<font color=gray><b>WAITING</b> <img src=images/bug/waiting.gif></font>";
                elseif($bug[2]['status'] == 4)
                    $value = "<font color=green><b>SUPPORTED</b></font> <img src=images/bug/ok.png>";
                elseif($bug[2]['status'] == 3)
                    $value = "<font color=red><b>NOT A BUG</b></font> <img src=images/bug/closed.png>";
                elseif($bug[2]['status'] == 1)
                    $value = "<font color=#4169E1><b>NEW ANSWER</b></font> <img src=images/bug/new.png>";
                ...
                $main_content .= '<TR BGCOLOR="'.$dark.'"><td width=40%><img src=images/bug/report.png> <b>Subject:</b></td><td> '.$tags[$bug[2]['tag']].' '.$bug[2]['subject'].'  '.$value.'</td></tr>';
                $main_content .= '<TR BGCOLOR="'.$light.'"><td><img src=images/bug/pri.gif> <b>Priority:</b></td><td> <img src=images/bug/'.$bug[2]['priority'].'.png> '.$priority[$bug[2]['priority']].'';
                $main_content .= '<TR BGCOLOR="'.$dark.'"><td><img src=images/bug/tibia.png> <b>Posted by:</b></td><td>';
                ...
                $main_content .= '</td></tr>';
                $main_content .= '<TR BGCOLOR="'.$light.'"><td colspan=2><img src=images/bug/des.png><b>Description:</b></td></tr>';
                ...
                {
                    if($answer['who'] == 1)
                        $who = "<img src=images/bug/staff.gif> <font color=red><b>SUPPORT</b></font>";
                    else
                    ...
                    $main_content .= '<TR BGCOLOR="'.$dark.'"><td width=70%><img src=images/bug/tibia.png><i><b>Posted by:</b></i></td><td>'.$who.'</td></tr>';
                    $main_content .= '<TR BGCOLOR="'.$light.'"><td colspan=2><img src=images/bug/des.png><i><b>Description:</b></i></td></tr>';
            ...
            else
            {
                if($bug[2]['status'] < 3)
                {
                    ...
                    $main_content .= '<br><form method="post" action=""><table><tr><td>Message:</i></td><td><textarea name="text" rows="3" cols="25"></textarea></td></tr><tr><td><br><font color=gray><b>IN PROGRESS</b></font> <img src=images/bug/waiting.gif></td><td><input type=radio name=status value=2></td></tr><tr><td><font color=green><b>SUPPORTED <img src=http://www.binarybiz.com/support/faq/admin/images/success.gif></b></font></td><td><input type=radio name=status value=4></td></tr><tr><td><font color=red><b>NOT A BUG <img src=http://www.iscr.com/imagenes/menu/cancel.gif></b></font></td><td><input type=radio name=status value=3></td></tr></table><br><input type="submit" name="finish" value="Submit" class="input2"/></form>';
                }
                else
                {
        ...
        if(!$post)
        {
            ...
            foreach($bug[1] as $report)
            {
                ...
                $main_content .= '<TR BGCOLOR="'.$bgcolor.'"><td width=75%><img src=images/bug/'.$report['priority'].'.png> <a href="index.php?subtopic=bugtracker&control=true&id='.$report['id'].'&acc='.$report['account'].'">'.$tags[$report['tag']].' '.$report['subject'].'</a></td><td>'.$value.'</td></tr>';
        ...
        if(!$post)
        {
            if($_REQUEST['add'] != TRUE)
            {
                ...
                foreach($bug[1] as $report)
                {
                    ...
                    $main_content .= '<TR BGCOLOR="'.$bgcolor.'"><td width=75%><img src=images/bug/'.$report['priority'].'.png> <a href="index.php?subtopic=bugtracker&id='.$report['id'].'">'.$tags[$report['tag']].' '.$report['subject'].'</a></td><td>'.$value.'</td></tr>';
            ...
            elseif($_REQUEST['add'] == TRUE)
            {
               ...
               $main_content .= '</td></tr><tr><td><br><img src=images/bug/pri.gif> <b>Priority:</b></td><td><br><select name="priority"><option value="">SELECT</option>';
